...
list_plots[[11]] <- ggplot(imgStats_b) +
    geom_boxplot(aes(as.character(cellCycle), vol_nucleus.p, fill = FoF)) + # , color = as.character(cellCycle))) +
    scale_fill_manual(values = c("FoF1_231005_fluorescent.nucleus" = "yellow", 
                                 "FoF2_231005_fluorescent.nucleus" = "red",
                                 "FoF3_231005_fluorescent.nucleus" = "green",
                                 "FoF4_231005_fluorescent.nucleus" = "blue")) +
...
descriptions <- c(
  ...
  "average pixel count per mitochondria",                                      # how is this different from the other pixels per mitochondria -- it is the average
...
cc_ <- xtable(table_df)
table_code <- print(cc_, type = "latex", include.rownames = FALSE, comment = FALSE, print.results = T)
