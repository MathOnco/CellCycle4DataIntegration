...
knitr::opts_knit$set(root.dir = "~/Repositories/CellCycle4DataIntegration/data4paper")
...
pseudotimeDirectionality <- function(pseudotime_img_, pseudotime_fucci_){
  pseudotime_fucci_ = pseudotime_fucci_[names(pseudotime_img_)]
  te= cor.test(pseudotime_img_,pseudotime_fucci_)
  par(mfrow=c(2,2))
  boxplot(pseudotime_img_ ~ pseudotime_fucci_)
  
  timeline=sort(unique(pseudotime_fucci_))
  la=grpstats(as.matrix(pseudotime_img_), pseudotime_fucci_, "mean")$mean[as.character(timeline),]
  te=list()
  for(shift in 1:length(timeline)){
    ...
    te[[shift]]= cor.test(la,timeline_)$estimate
  }
  te=do.call(rbind,te)
  ## First identify which pseudotimes must be reversed:
  iM=which.max(abs(te))
  if(te[iM]<0){
    pseudotime_img_ = pseudotime_img_ *-1
    pseudotime_img_ = pseudotime_img_ - min(pseudotime_img_)
    te[iM] = te[iM]*-1
  }
  
  
  ##sort all again 
  idx = order(pseudotime_img_)
  pseudotime_img_ = pseudotime_img_[idx]
  pseudotime_fucci_ = pseudotime_fucci_[idx]
  
  ## Next find max of circular cross correlation to shift pseudotime
  cMax=which.max(abs(te))
  cMax=cMax[cMax!=1]
  if(!isempty(cMax)){
    x=order(pseudotime_img_)
    idx=which(x==round(mean(x[pseudotime_fucci_>=cMax])) )
    x=x[c(idx:length(x), 1:(idx-1))]
    pseudotime_img_=pseudotime_img_[x]
  }
...
load(paste0(OUTPSEUDOTIME, filesep,"pseudotime_2025-03-12.RObj")) ## loads unsupervised pseudotime *model* list
...
```{r}
# Generate precursor images
# figname = paste0(OUTFIGS, filesep, "Fig1A_1.png")
# png(figname)
# bioimagetools::img(img[[1]][,,1]);
# dev.off()
# 
# figname = paste0(OUTFIGS, filesep, "Fig1A_3.png")
# png(figname)
...
# run("Red");
# run("RGB Color");
# imageCalculator("Add create", "Fig1A_1.png","Fig1A_3.png");
# selectImage("Result of Fig1A_1.png");
...
Figure 1B,C: Fucci based discrete cell cycle classification. 
```{r}
...
```{r}
tmp=plot_dimred(model[[paste0("Fucci_",FoF)]],color_cells = "pseudotime", size_cells=1.84,size_milestones=5)
tmp
...
```{r}
# Arrange data for applying custom continuous color scale
cc <-  fucci_ %>% 
...
```{r}
# Cells were selected manually for both spatial proximity and cycle state variation
...
list_plots <- vector('list', length(coi))
for (i in seq_along(list_plots)) {
  figname = paste0(OUTFIGS, filesep, "Fig2_", coi[i], "_", thedate, ".png")
  png(figname, width = 600,height = 600)
  par(mfrow=c(1,1), mai=c(0,0,0,0))
  
  col=which(cell==coi)
  a= coord__[coord__$id == coi[i],]
  ...
  a_ <-  a %>% 
    filter(id == coi[i], signal=="nucleus.p")
  ...
  a_ <-  a %>% 
    filter(id == coi[i], signal=="mito.p")
  plot3D::points3D(a_$x, a_$y, a_$z,pch3d=20,  axes=F, xlab="",ylab="", zlab="",col="yellow",alpha=0.2, add=T)
  
  a_ <-  a %>% 
    filter(id == coi[i], signal=="cytoplasm.p")
  plot3D::points3D(a_$x, a_$y, a_$z,pch3d=20,  axes=F, xlab="",ylab="", zlab="",col="cyan",alpha=0.2, add=T)
  
  dev.off()
  
  p <- magick::image_read(figname)
  plot <- magick::image_trim(p)
  list_plots[[i]] <- magick::image_ggplot(plot)
}

# Cells arranged correctly from left to right; relative spacing and image flip produced by hand for publication 
list_plots[[1]]+list_plots[[3]]+list_plots[[2]]
ggsave(paste0(OUTFIGS, filesep, "Fig3B_", thedate, ".png"))

list_plots[[4]]+list_plots[[5]]+list_plots[[6]]
ggsave(paste0(OUTFIGS, filesep, "Fig3C_", thedate, ".png"))
```
...
# saveAs("Tiff", "Fig3A_stack.tif");
# run("Slice Keeper", "first=14 last=14 increment=1");
# saveAs("Tiff", "Fig3A_slice.tif");
...
# Get center of cells, correct coordinates for image size
cent_df <- coord_ %>% 
  group_by(id) %>% 
  summarise(x = mean(x), y = mean(y)) %>% 
  mutate(x = 1024 - (x*4.3103), y = y*4.3103)

# Select coordinates for target cells
cent_df <- cent_df %>% 
  filter(id %in% coi)

# Assign labels for cells for figure
cent_labels <- cent_df[order(-cent_df$y),]
cent_labels <- cent_labels %>% 
  mutate(cell = row_number())

# Create labeled image for splicing into figure
...
# Create unlabeled image for main figure
...
```{r}
# Get list of features and cell cycle states
var_list <- svm$svmFeatures
cycle <- fucci_raw[rownames(imgStats),"cellCycle"]

# Merge cell feature data and cell cycle states with IDs
imgStats_b <- imgStats %>% 
  rownames_to_column(var = "long_ID") %>% 
  cbind(cycle) %>% 
  dplyr::select(c(long_ID, ID, FoF, cellCycle = cycle, var_list))

# Set colors per field
col <- c("FoF1_231005_fluorescent.nucleus" = "yellow", 
...
for (i in seq_along(list_plots)) {
  list_plots[[i]] <- ggplot(imgStats_b) +
    geom_boxplot(aes(as.character(cellCycle), .data[[var_list[i]]], fill = FoF)) +
...
# Add legend
list_plots[[11]] <- ggplot(imgStats_b) +
  geom_boxplot(aes(as.character(cellCycle), vol_nucleus.p, fill = FoF)) + 
  scale_fill_manual(values = col) +
  theme_light() +
...
d <- (list_plots[[11]] + list_plots[[10]])

# Generate figures
figname <- paste0(OUTFIGS, filesep, "Fig3D_",thedate,".png")
ggsave(figname,units="px",width = 480, height = 480, plot=list_plots[[1]], dpi=140)
la=plotPNGorTIF(figname)
...
graph <- charToRaw(graph)
rsvg::rsvg_png(graph, figname)
```
...
```{r}
tmp=plot_dimred(model$All,color_cells = "pseudotime", size_cells=1.84,size_milestones=5)
tmp
ggsave(paste0(OUTFIGS,filesep,"Fig4C_",thedate,"_labelFree_pseudotimeTSNE.png"),units="px",width = 0.9*480, height = 480, plot=tmp, dpi=100)
```
...
# Get fucci signals and coordinates
...
# Get data of target cells
target_cells <- target_df %>% 
  group_by(id) %>% 
  summarise(pseudotime = mean(pseudotime))

# Graph cells
target_cells_list <- unique(target_df$id)
list_plots <- vector('list', length(target_cells_list))
for(j in seq_along(list_plots)) {
  a <- coord_[coord_$id %in% target_cells_list[j],]
  a[,xyz] <- sweep(a[,xyz], MARGIN = 2, STATS = apply(a[,xyz], 2, min), FUN = "-")
  a[,xyz] <- 1 + a[,xyz]
  xyzlim <- apply(a[,xyz], 2, quantile, c(0,1))
  a$z <- z_interval * a$z
  a_ <-  a %>% 
    filter(id == target_cells_list[j], signal == "nucleus.p")
  figname <- paste0(OUTFIGS, filesep, "Fig4E_", target_cells_list[j], ".png")
  png(figname, width = 600, height = 600)
...
for(j in seq_along(list_plots)) {
  ...
  q <- target_cells %>% 
    filter(id == target_cells_list[j])
  xlab <- round(q$pseudotime[1], digits = 4)
  list_plots[[j]] <- plot +
    ggtitle(xlab)
}
...
png(paste0(OUTFIGS, filesep, "legend_3.png"), width = 300, height = 250)
plot.new()
lgd_ = rep(NA, 3)
lgd_[c(1,3)] = c("fucci_ch00", "fucci_ch02")
legend(x = 0, y = 1.02,
       legend = lgd_,
       fill = pal(3),
       border = NA,
...
p <- magick::image_read(paste0(OUTFIGS, filesep, "legend_3.png"))
plot <- magick::image_trim(p)
legend <- magick::image_ggplot(plot)


# Assemble figure: order determined manually for ascending pseudotime top to bottom, left to right
...
grid.arrange(
  arrangeGrob(
    ...
    list_plots[[5]], list_plots[[4]], list_plots[[3]], list_plots[[1]], list_plots[[15]], legend,
    ncol = 7,
    widths = unit(c(0.1, rep(0.5, 5), 0.3), "null")
  ),
  arrangeGrob(
    textGrob("B", x = 0, y = 1, vjust = 1, hjust = 0, gp = gpar(fontsize = 12, fontface = "bold")),
    list_plots[[14]], list_plots[[8]], list_plots[[6]], list_plots[[2]], list_plots[[9]], legend,
    ncol = 7,
    widths = unit(c(0.1, rep(0.5, 5), 0.3), "null")
  ),
  arrangeGrob(
    textGrob("C", x = 0, y = 1, vjust = 1, hjust = 0, gp = gpar(fontsize = 12, fontface = "bold")),
    list_plots[[10]], list_plots[[12]], list_plots[[7]], list_plots[[13]], list_plots[[11]],legend,
    ncol = 7,
    widths = unit(c(0.1, rep(0.5, 5), 0.3), "null")
  ),
  nrow = 3
...
# Merge coordinates with pseudotimes
...
# Add legends to cells with pseudotime ~0.9
p <- target_df %>% 
  filter(cell == "cell199")
xlab <- round(p[1,14], digits = 4)
...
tmp=plot_dimred(model$Seq,color_cells = "pseudotime", size_cells=1.84,size_milestones=5)
tmp
...
load(paste0(OUTPSEUDOTIME, filesep,"pseudotime_2025-03-12.RObj")); ## loads pq
pseudotimeDirectionality(model$All$pseudotime, imgStats, cellcyclecolumn="FUCCI_cellCycle")
...
stats_split=sapply(c("Seq","Img"), function(x) stats_[stats_$type==x,], simplify = F)
stats_split=sapply(names(stats_split), function(x) plyr::count(stats_split[[x]]$cluster)$freq, simplify = F)
sapply(stats_split, hist)
sapply(stats_split, mean)
sapply(stats_split, median)
sapply(stats_split, sum)

figname=paste0(OUTFIGS, filesep,"Fig5B_",thedate,".png") 
col=rainbow(max(stats_$cluster))
png(figname, width = 680)
ii=which(stats_$type=="Seq")
plot(1:length(ii), stats_$pseudotime[ii], col=col[stats_$cluster[ii]], xlab="cell", ylab="pseudotime", cex=2, pch=3, cex.axis=2, cex.lab=2)
ij=setdiff(1:nrow(stats_),ii)
ij=sort(sample(ij,length(ii)))
points(1:length(ij), stats_$pseudotime[ij], col=col[stats_$cluster[ij]], pch=1,cex=2)
...
```{r}
# # Correlation between gene expression and image features
# cc <- pval <- list()
# pairs=list()
...
#   
#   s = sapply(seqCells, function(x) mean(pq[x,sFeature], na.rm=T))
#   pairs[[sFeature]] = list()
#   for(iFeature in setdiff(colnames(imgStats), toExclude)){
#     i = sapply(imgCells, function(x) mean(imgStats[x,iFeature], na.rm=T ))
#     x=cor.test(s, i)
...
cc_=cc_[order(cc_$pval)[1:min(150,nrow(cc_))],]
...
# Sort and plot
poi=unique(unlist(apply(abs(cc),2, function(x) which(x>0.3)), use.names = F))
...
# Make custom dendrograms
distance = dist(as.matrix(hm_df), method = "manhattan")
cluster_row = hclust(distance, method = "ward.D")
distance = dist(t(as.matrix(hm_df)), method = "manhattan")
cluster_col = hclust(distance, method = "ward.D")
...
# Make a higher resolution key to splice onto heatmap
# hm=heatmap.2(as.matrix(hm_df), trace="n",
#              dendrogram = "none",
#              col = colorRampPalette(rev(brewer.pal(11, "Spectral"))),
#              cexCol = 0.1,
#              cexRow = 0.1)
# figname=paste0(OUTFIGS, filesep,"Fig5C_legend.png")
# png(figname, width = 1200, height = 700)
# eval(hm$call)
# dev.off()
...
tabname=paste0(OUTFIGS, filesep,"SupplementaryTable2_",thedate,".tex")
cc_ <- convert_matrix_to_df(cc, pval)
features <- unique(cc_$Cell.Feature)
...
descriptions <- c(
  ...
  "area of nucleus",
  "total pixel count of cytoplasm",
  "total pixel count of mitochondria",
  "total pixel count of nucleus",
  "number of cytoplasm DBSCAN clusters",
  "number of mitochondria DBSCAN clusters",
  "slice depth",
  "area of cell",
  "volume of cell",
  "height range of cell",                  
  "relative area of cell lost to indentations",                            
  "ratio of surface area to convex hull surface area cell",                                            
  "roundness of cell: spheres have a sphericity of 1",                     
  "second moment of surface area cell",                                                         
  "maximum area of available crossections of the cell",
  "degree of wrinkling of the cell",
  "total pixel count of cell",
  "height range of cytoplasm",
  "relative area of cytoplasm lost to indentations",                            
  "ratio of surface area to convex hull surface area cytoplasm",   
  "roundness of cytoplasm: spheres have a sphericity of 1",                     
  "second moment of surface area cytoplasm",                                                   
  "colony shape factor cytoplasm: high csf have high fragility",
  "Distance between cytoplasm and mitochondria (minimum)",
  "Distance between cytoplasm and mitochondria (maximum)",
  "Distance between cytoplasm and mitochondria (median)",
  "Distance between cytoplasm and nucleus (minimum)",
  "Distance between cytoplasm and nucleus (maximum)",
  "Distance between cytoplasm and nucleus (median)",
  "height range of mitochondria",               
  "relative area of mitochondria lost to indentations",                        
  "ratio of surface area to convex hull surface area mitochondria",                  
  "roundness of mitochondria: spheres have a sphericity of 1",                  
  "second moment of surface area mitochondria",                                                
  "colony shape factor mitochondria: high csf have high fragility",  
  "Distance between mitochondria and nucleus (minimum)",
  "Distance between mitochondria and nucleus (maximum)",
  "Distance between mitochondria and nucleus (median)",
  "height range of nucleus",
  "relative area of nucleus lost to indentations",                             
  "ratio of surface area to convex hull surface area nucleus",                                           
  "roundness of nucleus: spheres have a sphericity of 1",                        
  "second moment of surface area nucleus",                                                     
  "maximum area of available crossections of the nucleus",                      
  "degree of wrinkling of the nucleus",
  "average intensity of cytoplasm",
  "median intensity of cytoplasm",
  "maximum intensity of cytoplasm",
  "minimum intensity of cytoplasm",
  "average intensity of mitochondria",
  "median intensity of mutochondria",
  "maximum intensity of mitochondria",
  "minimum intensity of mitochondria",
  "average pixel count per mitochondria",                                      
  "resolution of mitochondria",
  "ratio of nuclei count to mitochondrial count",             
  "resolution of cytoplasm",
  "resolution of mutitochondria",
  "resolution of nucleus",
  "ratio of nuclear volume to nuclear area"
)

table_df <- data.frame("Imaging feature" = features, "Description" = descriptions)
cc_ <- xtable(table_df)
table_code <- print(cc_, type = "latex", include.rownames = FALSE, comment = FALSE, print.results = F)

modified_table <- gsub("\\\\begin\\{table\\}\\[ht\\]", "\\\\begin{longtable}[H]{| p{0.35\\\\textwidth} | p{.65\\\\textwidth}}", table_code)
modified_table <- gsub("\\\\end\\{table\\}", "\\\\caption{Definition of imaging features}\n \\\\label{table_imgfeature_def}\n \\\\end{longtable}", modified_table)
modified_table <- gsub("\\\\centering", "", modified_table)
modified_table <- gsub("\\\\begin\\{tabular\\}\\{ll\\}", "", modified_table)
modified_table <- gsub("\\\\end\\{tabular\\}", "", modified_table)
writeLines(modified_table, tabname)
```

## Assemeble all figures into panels. 
